...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    token_to_id = {
        special:idx for idx, special in enumerate(specials)
    }
    last_idx = len(specials) - 1
...
def build_id_to_token_vocab(token_to_id):
    ...
    return {
        id: token for token, id in token_to_id.items()
    }
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    words = sentence.split()
    for word in words:
        tokens.append(token_to_id.get(word, token_to_id[unk_token]))
    return tokens
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    ids.extend([pad_id for i in range(max_len - len(ids))])
    return ids[:max_len]
...
def scale_embeddings_by_sqrt_d_model(embeddings, d_model):
    ...
    return embeddings * torch.sqrt(torch.tensor(d_model))
